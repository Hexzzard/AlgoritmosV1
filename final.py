...
def anadir_huelgura(renglon, h, rengArr):
...
def miVersion(x):
    ...
    for i in range(2, len(lines)):
        ...
        xF.append([float(y[j][0]) if j < vars and y[j][0] != '' else
                   ...
                   y[j][len(y[0]) - 1] if j == (len(y) - 2) else
                   0 for j in range(len(y))])
    ...
    z = [-float(i[0]) if max and hayMayor else -float(i[0]) if not max and not hayMayor else -float(i[0]) if not hayMayor and max else float(i[0]) for i in
         re.findall(pattern, lines[0])]
    ...
    """
    try:
        zArtificial = anadir_huelgura(zArtificial, xHuelg, rengArr)
        zArtificial.append(0)
        z = anadir_huelgura(z, xHuelg, rengArr)
        z.append(0)
        xHuelg += 1
    ...
    for i in range(len(xF)):
        ...
        reng = anadir_huelgura([xF[i][j] for j in range(vars)], xHuelg, rengArr)
...
1x2 <= 30
"""
input_str3 = """maximize 30000x1 + 50000x2 
subject to
3x1 - 2x2 <= 18
2x2 <= 12
1x1 <= 4
"""
input_str4 = """maximize 2x1 + 5x2 
subject to
1x1 + 6x2 <= 20
1x1 + 1x2 <= 60
1x1 <= 40
...
for i in range(var):
    print(f'x{i + 1}={LDx[i]}')
iSombra = sombra = 0
for i in range(var, len(matt[0]) - 1):
    if matt[0][i] > sombra:
        sombra = matt[0][i]
        iSombra = i
print(str(VBv[iSombra]) + " = " + str(sombra))
print("Valor final función obj: " + str(matt[0][len(matt[0])-1]))